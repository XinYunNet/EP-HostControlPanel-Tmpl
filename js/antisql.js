...
function copy_code()
{
	document.getElementById('param').value = "'.*[; ]?((or)|(insert)|(select)|(union)|(update)|(delete)|(replace)|(create)|(drop)|(alter)|(grant)|(load)|(show)|(exec))[\s(].*(exec|insert|truncate|char|declare|union|replace|alter|grant|etc/|win.ini)+.*";
	close_msg();
}
function piao_msg()
{
	...
	var msg = "正则输入，如需防sql注入，可用以下正则:<br>";
		msg += "'.*[; ]?((or)|(insert)|(select)|(union)|(update)|(delete)|(replace)|(create)|(drop)|(alter)|(grant)|(load)|(show)|(exec))[\s(].*(exec|insert|truncate|char|declare|union|replace|alter|grant|etc/|win.ini)+.*<br>";
